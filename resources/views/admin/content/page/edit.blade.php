...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb font-size-12">
        ...
        <li class="breadcrumb-item active" aria-current="page">ویرایش پیج جدید</li>
    </ol>
</nav>
<div class="col-md-12 mt-4">
    <div class="content">
        <h4>ویرایش پیج جدید</h4>
        <div class="d-flex justify-content-between align-items-center my-3">
            <a href="{{ route('admin.content.page.index') }}" class="btn btn-info btn-sm">بازگشت</a>
        </div>
        <form class="row" action="{{ route('admin.content.page.update',[$page->id]) }}" method="post">
            @csrf
            {{ method_field('put') }}
            <div class="col-md-6 mb-2">
                <fieldset class="form-group">
                    <label for="title">عنوان صفحه</label>
                    <input class="form-control form-control-sm" value="{{ old('title', $page->title) }}" name="title" type="text" placeholder="عنوان صفحه ...">
                </fieldset>
                @error('title')
            ...
            <div class="col-md-6 mb-2">
                <fieldset class="form-group">
                    ...
                    <input class="form-control form-control-sm" value="{{ old('url', $page->url) }}" name="url" type="text" placeholder="آدرس صفحه ...">
                </fieldset>
                @error('url')
                <small class="text-danger">{{ $message }}</small>
            @enderror
            ...
            <div class="col-12 mb-2">
				<fieldset class="form-group">
					...
					<textarea class="form-control form-control-sm" id="body" name="body" cols="6">{{ old('body', $page->body) }}</textarea>
				</fieldset>
                @error('body')
                <small class="text-danger">{{ $message }}</small>
            @enderror
